...
class qa_reports_monthly_page {

	function match_request($request)
	{
		$parts=explode('/', $request);

		return $parts[0]=='reports-monthly' && (qa_get_logged_in_level()>=QA_USER_LEVEL_ADMIN);
	}
	...
	function process_request($request)
	{
        ...
        if (!$startDate && !$endDate) {     //default to current and last month
			$endDate = date('Y-m');
		...
		$qa_content['custom_report_form'] = '<form method="POST" autocomplete="off" action="'.qa_self_html().'">
			<label>Start Month: <input type="month" name="startDate" id="startMonth" value="'.$startDate.'" max="'.date('Y-m').'"></input></label>
			<label>End Month: <input type="month" name="endDate" id="endMonth" value="'.$endDate.'" max="'.date('Y-m').'" /></label>
		...
		if (($startDate && !$endDate) || (!$startDate && $endDate)){
            ...
            //Because enddate is not inclusive for the sql query, we must add a month
            $formattedEndDate = DateTime::createFromFormat('Y-m', $endDate)->getTimestamp();
            $endDate = date("Y-m", strtotime("+1 month", $formattedEndDate));
